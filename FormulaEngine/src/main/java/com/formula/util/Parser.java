...
public class Parser {
	...
	static private final String[] procs1 = { "abs", "acos", "asin", "atan", "ceil", "cos", "exp", "floor", "log",
			"round", "sin", "sqrt", "tan", "neg" };
	static private final int[] rators1 = { Expr.ABS, Expr.ACOS, Expr.ASIN, Expr.ATAN, Expr.CEIL, Expr.COS, Expr.EXP,
			Expr.FLOOR, Expr.LOG, Expr.ROUND, Expr.SIN, Expr.SQRT, Expr.TAN , Expr.NEG};

	static private final String[] procs2 = { "atan2", "max", "min" };
	static private final int[] rators2 = { Expr.ATAN2, Expr.MAX, Expr.MIN };
	
	static private final String[] procs3 = {"maximum","minimum","average","count","sum" };
	static private final int[] rators3 = { Expr.MAXIMUM, Expr.MINIMUM ,Expr.AVERAGE, Expr.COUNT, Expr.SUM};
	
	static private final String[] procs4 = {"SingleLevelInterpolation","TowLevelInterpolation","ThreeLevelInterpolation","FourLevelInterpolation"};
	static private final int[] rators4 = { Expr.SINGLELEVELINTERPOLATION, Expr.TWOLEVELINTERPOLATION, Expr.THREELEVELINTERPOLATION, Expr.FOURLEVELINTERPOLATION};
	...
	private Expr parseFactor() throws SyntaxException {
		...
		case Token.TT_WORD: {
			...
			for (int i = 0; i < procs3.length; ++i)
				if (procs3[i].equals(token.sval)) {
					nextToken();
					expect('(');
					Expr rand = parseExpr(0);
					expect(')');
					return Expr.makeApp3(rators3[i], rand);
				}  
			
		 	for (int i = 0; i < procs4.length; ++i){
				if (procs4[i].equals(token.sval) && token.sval.equalsIgnoreCase("SingleLevelInterpolation") ) {
					nextToken();
					expect('(');
					Expr rand1 = parseExpr(0);
					expect(',');
					Expr matrixId = parseExpr(0);
					expect(')');
					return Expr.makeApp4(rators4[i], rand1,matrixId);
				}
				if (procs4[i].equals(token.sval) && token.sval.equalsIgnoreCase("TowLevelInterpolation") ) {
					nextToken();
					expect('(');
					Expr rand1 = parseExpr(0);
					expect(',');
					...
					Expr matrixId = parseExpr(0);
					expect(')');
					return Expr.makeApp5(rators4[i], rand1,rand2,matrixId);
				}
				if (procs4[i].equals(token.sval) && token.sval.equalsIgnoreCase("ThreeLevelInterpolation") ) {
					nextToken();
					expect('(');
					Expr rand1 = parseExpr(0);
					expect(',');
					Expr rand2 = parseExpr(0);
					expect(',');
					Expr rand3 = parseExpr(0);
					expect(',');
					Expr matrixId = parseExpr(0);
					expect(')');
					return Expr.makeApp6(rators4[i], rand1,rand2,rand3,matrixId);
				}
				if (procs4[i].equals(token.sval) && token.sval.equalsIgnoreCase("FourLevelInterpolation")) {
					nextToken();
					expect('(');
					Expr rand1 = parseExpr(0);
					expect(',');
					...
					Expr rand3 = parseExpr(0);
					expect(',');
					Expr rand4 = parseExpr(0);
					expect(',');
					Expr matrixId = parseExpr(0);
					expect(')');
					return Expr.makeApp7(rators4[i], rand1,rand2,rand3,rand4,matrixId);
				}
		}
	...
	private void expect(int ttype) throws SyntaxException {
		if (token.ttype != ttype)
			throw error("'" + (char) ttype + "' expected", SyntaxException.EXPECTED, "" + (char) ttype);
		nextToken();
	}
